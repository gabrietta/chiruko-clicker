...
- 自動停止（いわゆる誤BAN）は廃止しています。異常な計算結果が出た場合はその1回の更新だけを無視し、ゲームと自動生産は継続します。旧版で停止していたセーブも、更新後の起動時に進行を失わず自動復旧します。スマートフォンの二重タップは無視され、短時間に救済が集中した場合は救済イベントだけが少し休憩します。
