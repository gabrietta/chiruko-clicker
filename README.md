...
- 旧版で奇跡報酬の累計判定により一時停止したセーブは、更新後の起動時に自動復旧します。それ以外の安全確認で一時停止した場合も、画面上の「設定を開いて再開」から、満足・設備・実績・救済印を減らさず再開できます。スマートフォンの二重タップは無視され、短時間に救済が集中した場合は救済イベントだけが少し休憩します。
